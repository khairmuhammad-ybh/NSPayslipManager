...
import {
  payslipTemplateDbOptions,
  payslipDbOptions,
  payslipComparedDbOptions,
} from '../datasource/dbconfig.datasource';
import {
  payslipTemplateSchema,
  payslipSchema,
  payslipComparedSchema,
} from '../datasource/dbmodel.datasource';
  ...
  return new Promise((resolve, reject) => {
    Realm.open(payslipTemplateDbOptions)
      .then(realm => {
        ...
        let payslips = payslipTemplates.filtered(
          `timeStamp = \"${newPayslipTemplate.timeStamp}\"`,
        );
  ...
  return new Promise((resolve, reject) => {
    Realm.open(payslipTemplateDbOptions)
      .then(realm => {
        ...
        let retrievePayslipsTemplate = realm.objects(
          payslipTemplateSchema.name,
        );
  ...
  return new Promise((resolve, reject) => {
    Realm.open(payslipComparedDbOptions)
      .then(realm => {
        ...
        let comparedPayslips = allComparedPayslips.filtered(
          `timeStamp = \"${newComparedPayslip.timeStamp}\"`,
        );
  ...
  return new Promise((resolve, reject) => {
    Realm.open(payslipComparedDbOptions)
      .then(realm => {
        ...
        let retrieveComparedPayslips = realm.objects(
          payslipComparedSchema.name,
        );
  ...
  return new Promise((resolve, reject) => {
    Realm.open(payslipComparedDbOptions)
      .then(realm => {
        realm.write(() => {
          let allComparedPayslips = realm.objects(payslipComparedSchema.name);
          let filteredPayslip = allComparedPayslips.filtered(
            `_id = \"${_id}\"`,
          );
          realm.delete(filteredPayslip);
        });
